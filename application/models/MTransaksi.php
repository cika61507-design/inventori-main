...
{
    $query = $this->db->query("SELECT tt.*, tb.nama_barang, tb.id_barang AS nomorbarang 
    FROM tbl_transaksi_masuk AS tt
    LEFT JOIN tbl_barang tb
    ON tt.id_barang = tb.pk_barang_id
    ORDER BY tt.pk_transaksi_masuk_id DESC");
...
{
    $query = $this->db->query("SELECT tt.*, tb.nama_barang, tb.id_barang AS nomorbarang 
    FROM tbl_transaksi_keluar AS tt
    LEFT JOIN tbl_barang tb
    ON tt.id_barang = tb.pk_barang_id
    ORDER BY tt.pk_transaksi_keluar_id DESC");
...
function get_riwayat_masuk($id_barang)
{
    return $this->db
        ->where('id_barang', $id_barang)
        ->order_by('tanggal', 'ASC')
        ->get('tbl_transaksi_masuk')
        ->result();
}
